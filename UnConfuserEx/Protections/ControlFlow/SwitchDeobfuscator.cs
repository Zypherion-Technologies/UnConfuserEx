        ...
        private bool IsSwitchBlock(Block curBlock)
        {
            ...
            for (int i = 0; i < instrs.Count; i++)
            {
                    ...
                    cases.Length == instrs[i - 2].GetLdcI4Value())
                {
                    ...
                    if (instrs[i - 5].OpCode == OpCodes.Call && instrs[i - 5].Operand is MethodDef predicate)
                    {
                        methodOffset = i - 5;
                        x86Method = new X86Method(Module, predicate);
                    }
        ...
        private bool DeobfuscateSwitchBlock()
        {
            ...
            do
            {
                ...
                if (x86Method != null && switchBlock!.FirstInstr.IsLdcI4())
                {
                    ...
                    {
                        if (source.FallThrough == switchBlock)
                        {
                            source.SetNewFallThrough(nextBlock);
                            modified = true;
                        }
                    }
