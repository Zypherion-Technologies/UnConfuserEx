...
namespace UnConfuserEx.Protections.Compressor.RuntimeLzma {
	public static class Lzma {
		...
		public static byte[] Decompress(byte[] data) {
			...
			while (offset < 5)
				offset += s.Read(prop, offset, 5 - offset);
			...
			while (offset < 4)
				offset += s.Read(sizeBuf, offset, 4 - offset);

			if (!BitConverter.IsLittleEndian)
				Array.Reverse(sizeBuf, 0, 4);

			int sz = BitConverter.ToInt32(sizeBuf, 0);
			var b = new byte[sz];
			var z = new MemoryStream(b, true);
			long compressedSize = s.Length - 5 - 4;
			decoder.Code(s, z, compressedSize, (long)sz);
			return b;
		}
