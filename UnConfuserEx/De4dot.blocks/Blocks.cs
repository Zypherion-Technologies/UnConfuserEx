...
namespace de4dot.blocks {
	public class Blocks {
		static readonly ILog Logger = LogManager.GetLogger("Blocks");
		MethodDef method;
		IList<Local> locals;
		...
		public void RepartitionBlocks() {
			...
			foreach (var scopeBlock in GetAllScopeBlocks(methodBlocks)) {
				try {
					...
					Logger.Warn($"Null ref exception! Invalid metadata token in code? Method: {method.MDToken.Raw:X8}: {method.FullName}");
					return;
				}
			}
		}
